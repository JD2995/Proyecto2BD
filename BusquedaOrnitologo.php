...
<html lang="es">
	...
	<body>
			  ...
			  <!--<label for etiqueta ="titulo">Registrate!</label>-->
					<div class="container">
						<div class="page-header">
							<center><h2>Consultas de Ornitólogos</h2></center>
						</div>
					</div>	
		...
		if(isset($_GET['tipos'])){
			...
			if($_GET['buscar'] == ""){
				$arrayPersonas= obtenerOrnitologos();
			}
			else{
				//Obtiene los resultados dependiendo del tipo de búsqueda
				if($_GET['tipos'] == "Nombre"){
					$arrayPersonas= obtenerOrnitologosNombre($_GET['buscar']);
				}
				else if($_GET['tipos'] == "Apellido"){
					$arrayPersonas= obtenerOrnitologosApellido($_GET['buscar']);
				}
				else if($_GET['tipos'] == "Correo"){
					$arrayPersonas= obtenerOrnitologosCorreo($_GET['buscar']);
				}
		...
		while($indPersona < count($arrayPersonas)){
			...
			if(!in_array($arrayPersonas[$indPersona][0],$arrayID)){
				/*$ind= $arrayPersonas[$indPersona][0];
				var_dump($arrayPersonas);
				$cantFotos= obtenerCantFotos($ind);*/
				print "<tr>".
				...
				"<td><button class=\"btn btn-default\" onclick=\"verPerfil(".$arrayPersonas[$indPersona][0].",1)\">Ver perfil</button></td></tr>";
